<script lang="ts">
  ...
  async function vote(thisid, votepayload) {
    status = "voting";
    let res = await $daoActor.vote(BigInt(thisid), votepayload)
    if (res.Ok) {
      return res.Ok
    } else {
  ...
  async function get_proposal(thisid) {
    status = "prop_getting";
    id = thisid
    let res = await $daoActor.get_proposal(BigInt(thisid))
    if (res.Ok) {
      return res.Ok
    } else {
        throw new Error(res.Err)
  ...
  function handleVoteClick(choosenvote) {
    promise = vote(id, choosenvote)
    $hasvoted = true
  }
...
<div class="votemain">
  {#if $principal}
    ...
    {#if $proposaltoVote.proposalID == "null"}
      <h1 class="slogan">Please check "View" to see proposals!</h1>
        ...
        <div class="votingdiv">
          ...
          <div>
            ...
            {#if $hasvoted === true}
              ...
              {:then res2}
                <p style="color: white">
                  Voted successfully! Current votes: YES: {res2[0]} ; NO:  {res2[1]}
                </p>
              {:catch error}
